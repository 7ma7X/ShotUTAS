...
module.exports = class {
     ...
     * @param {String} path
     */
    static async search (keyword, browser, page, filepath) {
        await page.type('#freeKeyword', keyword);
        page.click('input[value=" 検索 "]');
        await page.waitForNavigation({timeout: 30000, waitUntil: 'networkidle0'}).catch(async (err) => {
        ...
        await page.$('.error').then(async (err) => {
            if (err) {
                ...
                while (true) {
                    ...
                    await this.takeShot(num, rows, browser, path);
            ...
            this.search(searchWord, browser, page, filepath);
        });
    }
     ...
     * @param {Browser} browser 
     */
    static async takeShot (num, rows, browser, filepath) {
        ...
        await utilities.mkdir(path.join(filepath, subject));
        await element.screenshot({path: path.join(filepath, subject, `${subject}_pic1.png`)});
        ...
        await element2.screenshot({path: path.join(filepath, subject, `${subject}_pic2.png`)});
